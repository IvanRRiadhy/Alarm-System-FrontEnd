...
import { defaultAlarmRuleFilter } from 'src/store/apps/defaultForm';
...
const AlarmRuleFilter = () => {
  ...
  const [appliedFilter, setAppliedFilter] = useState(alarmRuleFilter);

  // --- Sync with store ---
  useEffect(() => {
    setAppliedFilter(alarmRuleFilter);
  }, [alarmRuleFilter]);
  ...
  const handleStatusChange = (val: any) => {
    setAppliedFilter((prev) => ({
      ...prev,
      isActive: val?.value ?? undefined,
    }));
  };

  // --- Apply & Reset ---
  const handleApplyFilter = () => {
    dispatch(UpdateAlarmRuleFilter({ ...appliedFilter, page: 1 }));
    setOpen(false);
  };

  const handleResetFilter = () => {
    setAppliedFilter(defaultAlarmRuleFilter);
    dispatch(UpdateAlarmRuleFilter({ ...defaultAlarmRuleFilter }));
    setOpen(false);
  };
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={12}>
            ...
            <CustomAutocomplete
              ...
              value={statusOptions.find((s) => s.value === appliedFilter.isActive) || null}
          ...
          <Grid container justifyContent="space-between">
            ...
            <Grid size={6}>
              <Button
                ...
                disabled={isEqual(appliedFilter, alarmRuleFilter)}
              >
                Apply
              </Button>
            </Grid>
...
export default AlarmRuleFilter;
